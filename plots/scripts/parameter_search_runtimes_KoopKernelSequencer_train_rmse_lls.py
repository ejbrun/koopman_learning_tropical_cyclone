...
for idx_row, (mask_koopman_operator, use_nystroem_context_window) in enumerate(
    ...
    runtime = []
    for koopman_kernel_length_scale in plot_settings["koopman_kernel_length_scale"]:
        ...
        runtime.append(res_dict[conf]["training_runtime"])
    ax[idx_row, 0].plot(
        plot_settings["koopman_kernel_length_scale"],
        runtime,
        label=str((context_mode, mask_koopman_operator, use_nystroem_context_window)),
    )
    ...
    if not mask_koopman_operator:
        ...
        ax[idx_row, 1].plot(
            ...
            runtime,
            label=str(
                (context_mode, mask_koopman_operator, use_nystroem_context_window)
            ),
        )
...
fig.savefig("parameter_search_runtimes_KoopKernelSequencer_train_rmse_lls.pdf")
