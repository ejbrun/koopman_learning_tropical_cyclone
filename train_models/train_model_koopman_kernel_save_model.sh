...
do
    ...
    do
        ...
        do
            for basin in ${basins[@]};
            do
                python train_model_koopman_kernel_save_model.py --model=Nystroem_RRR --basin=$basin --year_range=1980,2021 --koopman_kernel_length_scale=$kk_length_scale --koopman_kernel_rank=$kk_krank --koopman_kernel_num_centers=$kk_ncenters --koopman_kernel_num_train_stops=$kk_num_train_stops --tikhonov_reg=$tikhonov_reg
            done
        done
    done
done
